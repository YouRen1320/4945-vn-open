...
> 制定日期：2026-08-05。
>
> 现实基线：`4.0.0` 已于 2026-08-13 正式发布，前三季实现及现有自动化门禁已经落地；`4.1–5.0` 仍是条件规划，不能据此声称第四季已经立项。
>
...
前三项已经随 `4.0.0` 落地，并由当前代码和自动化测试持续校验；第四季 postmortem、立项决定和 canon 签认仍未完成。当前客户端仍把 auto、quick 和 6 个手动槽保存在 `localStorage`，尚无 IndexedDB。第四季实现前必须重新测量真实序列化体积、浏览器配额和失败行为，不能把本规划中的估算写成验证结论。
